...
from GenealogicalMutationOperator import *
from GaussianMutationOperator import *
from GeneticBiOperator import *
...
def logStatistics(aNumberOfIndividuals):
    ...
    global mutation_bi_operator;

    if not isinstance(args.logging, NoneType):
        if g_first_log:
            g_first_log = False;

            logging.info("generation,new_individual_counter,event,number_of_individuals,number_of_lamps,global_fitness,enlightment,overlap,good_flies,bad_flies,delta_global_fitness_accumulator_1,delta_global_fitness_accumulator_2,counter_1,counter_2,alpha");
        ...
        logging.info("%i,%i,%s,%i,%i,%f,%f,%f,%f,%f,%f,%f,%i,%i,%f" % (
            g_iteration,
            ...
            good_flies,
            bad_flies,
            mutation_bi_operator.delta_global_fitness_accumulator_1,
            mutation_bi_operator.delta_global_fitness_accumulator_2,
            mutation_bi_operator.counter_1,
            mutation_bi_operator.counter_2,
            mutation_bi_operator.alpha
        ));
...
try:
    ...
    gaussian_mutation = GaussianMutationOperator(0.0, args.initial_mutation_variance[0]);
    genealogical_mutation = GenealogicalMutationOperator(0.0);
    mutation_bi_operator = GeneticBiOperator(0.7, gaussian_mutation, genealogical_mutation, 0.5, ObjectiveFunction.ObjectiveFunction.MAXIMISATION, max(10, int(number_of_individuals / 2)));


    # Add the genetic operators to the EA
    optimiser.addGeneticOperator(new_blood);
    #optimiser.addGeneticOperator(gaussian_mutation);
    #optimiser.addGeneticOperator(genealogical_mutation);
    optimiser.addGeneticOperator(mutation_bi_operator);
